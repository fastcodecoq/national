<?php

/* ---------------------------------------------------------------------- */
/*	Contact Form
/* ---------------------------------------------------------------------- */

// The email address where all contact form messages are sent (inbox)
$your_email_address = 'info@nationalluxurycoach.com';

// The message reply for the client
$thanks_message  = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <p>Thank you for contacting National Luxury Coach. Your email has been sent & a National Luxury Coach representative will be in contact with you shortly.</p>';


// URL of the email Texture Background image

$url_texture_background = '';                   

/* ---------------------------------------------------------------------- */
/*	Latest Tweets
/* ---------------------------------------------------------------------- */

// The Twitter account
$your_twitter_username = '';

// How many tweets will be displayed
$tweets_limit = 2;

// Do you want to show replies
$ignore_replies = true;

// Date settings
$twitter_style_dates = true; // e.g. "about 1 hour ago"

// Date setting if $twitter_style_dates is false
$date_format = 'F d, Y'; // e.g. "December 20, 2011"

// Seconds to cache feed (1 hour)
$twitter_cachetime = 60*60;

/* ---------------------------------------------------------------------- */
/*	Latest Flickr Images
/* ---------------------------------------------------------------------- */

// The Flickr account ID
$your_flickr_id = ''; // Get your Flickr ID at http://idgettr.com/

// How many images will be displayed
$flickr_limit = 6;

// Seconds to cache feed (1 hour)
$flickr_cachetime = 60*60;